...
void BaseEngine::CreateObjectArray(int iNumberObjects)
{
	...
	displayableObjectList = new std::vector<DisplayableObject*>;
	displayableObjectList->reserve(iNumberObjects);
}
...
void BaseEngine::StoreObjectInArray(int iIndex, DisplayableObject* pObject)
{
	if (pObject == NULL)
	{
		// Remove the iIndex to size() elements
		auto it = displayableObjectList->begin() + iIndex;
		for (; it <= displayableObjectList->end(); it++)
		{
			delete dynamic_cast<DisplayableObject*>(*it);
			displayableObjectList->erase(it);
		}
	}
	else
	{
		// Set the value of the iIndex element
		auto it = displayableObjectList->begin();
		displayableObjectList->insert(it + iIndex, pObject);
	}
...
{
	if (pObject != NULL)
		displayableObjectList->push_back(pObject);
}

void BaseEngine::RemoveObjectFromArray(DisplayableObject* pObject)
{
	delete pObject;
	displayableObjectList->erase(std::remove(displayableObjectList->begin(), displayableObjectList->end(), pObject), displayableObjectList->end());
}

void BaseEngine::ApplyToAllObjects(void(*func)(DisplayableObject*))
{
	if (displayableObjectList != NULL)
	{
		for (int i = 0; i < displayableObjectList->size(); i++)
		{
			func((*displayableObjectList)[i]);
		}
	}
}
...
void BaseEngine::DestroyOldObjects()
{
	...
	if (displayableObjectList != NULL)
	{
		for (int i = 0; i < displayableObjectList->size(); i++)
		{
			delete (*displayableObjectList)[i];
		}
		delete displayableObjectList;
	}
}
...
void BaseEngine::UpdateAllObjects( int iCurrentTime )
{
	...
	if ( displayableObjectList != NULL )
	{
		...
		for ( int i = 0 ; i < displayableObjectList->size(); i++ )
		{
			(*displayableObjectList)[i]->DoUpdate(iCurrentTime);
...
void BaseEngine::UndrawObjects()
{
	...
	if ( displayableObjectList != NULL )
	{
		for ( int i = 0 ; i < displayableObjectList->size(); i++ )
		{
			(*displayableObjectList)[i]->RedrawBackground();
			if ( m_iDrawableObjectsChanged )
				return; // Abort! Something changed in the array
		}
	}
...
void BaseEngine::DrawObjects()
{
	...
	if ( displayableObjectList != NULL)
	{
		for (int i = 0; i < displayableObjectList->size(); i++)
		{
			displayableObjectList->at(i)->Draw();
			if ( m_iDrawableObjectsChanged )
				return; // Abort! Something changed in the array
		}
	}
...
{
	if (displayableObjectList != NULL && displayableObjectList->size() > iIndex)
	{
		return (*displayableObjectList)[iIndex];
	}
	else
		return NULL;
}
...
void BaseEngine::NotifyAllObjects( int iSignalNumber )
{
	if (displayableObjectList != NULL)
	{
		for (int i = 0; i < displayableObjectList->size(); i++)
		{
			(*displayableObjectList)[i]->Notify( iSignalNumber );
		}
	}
}
...
int BaseEngine::NotifyAllObjectsGetCountNonZero( int iSignalNumber )
{
	...
	if (displayableObjectList != NULL)
	{
		for (int i = 0; i < displayableObjectList->size(); i++)
		{
			if ( (*displayableObjectList)[i]->Notify( iSignalNumber ) != 0 )
				iReturn++;
		}
	}
	return iReturn;
...
int BaseEngine::NotifyAllObjectsGetSum( int iSignalNumber )
{
	...
	if (displayableObjectList != NULL)
	{
		for (int i = 0; i < displayableObjectList->size(); i++)
		{
			iReturn += (*displayableObjectList)[i]->Notify( iSignalNumber );
		}
	}
	return iReturn;
}
...
int BaseEngine::NotifyAllObjectsGetMax( int iSignalNumber )
{
	...
	if (displayableObjectList != NULL)
	{
		for (int i = 0; i < displayableObjectList->size(); i++)
		{
			int ival = (*displayableObjectList)[i]->Notify(iSignalNumber);
			if ( ival > iReturn )
				iReturn = ival;
		}
	}
