...
public:
	...
	/* Store an object in the next available space */
	void StoreObjectInArray(DisplayableObject* pObject);

	/* Remove an object from the array */
	void RemoveObjectFromArray(DisplayableObject* pObject);

	/* Apply a function to all DisplayableObjects */
	void ApplyToAllObjects(void(*func)(DisplayableObject*));

	/* Get all DisplayableObjects of a particular sub-type */
	template <class T>
	std::vector<T*> GetAllObjectsOfType()
	{
		...
		if (displayableObjectList != NULL)
		{
			...
			{
				T* obj = dynamic_cast<T*>((*displayableObjectList)[i]);
				// If the object is not of type T, the cast will return NULL
				if (obj != NULL)
...
private:
	...
	std::vector<DisplayableObject*> *displayableObjectList = NULL;
